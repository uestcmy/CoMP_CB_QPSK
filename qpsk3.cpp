    ...
    ui(new Ui::QPSK3)
{
    ...
    InputManagement();
       ...
       addrSrv_chl1.sin_port=htons(8007);//server : receive port number
...
*/
    ...
    double hw42_re_rx2[4][2];
    double hw42_im_rx2[4][2];
    ...
    for( int t = 0 ; t < 3 ; t++){//time
        for(int f = 0 ; f < 4 ; f ++){//freq
            ...
            for( int i = 0 ; i < 4 ; i++ ){
                for( int j = 0 ; j < 2 ; j++){
                    hw42_re_rx4[i][j] += hw42_re[i][j];
                    hw42_im_rx4[i][j] += hw42_im[i][j];
                }
            }

            for( int i = 0 ; i < 4 ; i++ ){
                for( int j = 0 ; j < 2 ; j++ ){
                    hw42_re_rx4[i][j]  *= norm4;
                    hw42_im_rx4[i][j]  *= norm4;
                }
            }
            qDebug()<<"rx4 re";
            for( int i = 0 ; i < 4 ; i++ ){
...
*/
    ...
    for(int i=0;i<4;i++){
        for(int j=0;j<2;j++){
            hw2_42_re[i][j] = hw42_re_rx4[i][j];
            hw2_42_im[i][j] = hw42_im_rx4[i][j];
        }
    }
        hw2_42_re[3][1]=(hw42_re_rx4[3][1]*hw42_re_rx4[2][0]+hw42_im_rx4[3][1]*hw42_im_rx4[2][0])/(hw42_re_rx4[2][0]*hw42_re_rx4[2][0]+hw42_im_rx4[2][0]*hw42_im_rx4[2][0]);
        hw2_42_im[3][1]=(hw42_re_rx4[3][1]*hw42_im_rx4[2][0]-hw42_im_rx4[3][1]*hw42_re_rx4[2][0])/(hw42_re_rx4[2][0]*hw42_re_rx4[2][0]+hw42_im_rx4[2][0]*hw42_im_rx4[2][0]);

        hw2_42_re[3][0]=(hw42_re_rx4[3][0]*hw42_re_rx4[2][0]+hw42_im_rx4[3][0]*hw42_im_rx4[2][0])/(hw42_re_rx4[2][0]*hw42_re_rx4[2][0]+hw42_im_rx4[2][0]*hw42_im_rx4[2][0]);
        hw2_42_im[3][0]=(hw42_re_rx4[3][0]*hw42_im_rx4[2][0]-hw42_im_rx4[3][0]*hw42_re_rx4[2][0])/(hw42_re_rx4[2][0]*hw42_re_rx4[2][0]+hw42_im_rx4[2][0]*hw42_im_rx4[2][0]);
...
/*
     ...
     for( int t = 0 ; t < 3 ; t++){//time
        for(int f = 0 ; f < 4 ; f ++){//freq
            ...
            Matrix_mult482(mat48_2_re,mat48_2_im,w82_re,w82_im,hw42_re_rx2,hw42_im_rx2);//hw
            ...
            for( int i = 0 ; i < 4 ; i++ ){
                for( int j = 0 ; j < 2 ; j++){
                    hw42_re[i][j] += hw42_re_rx2[i][j];
                    hw42_im[i][j] += hw42_im_rx2[i][j];
                }
            }
            ...
            for (int i=0;i<2;i++){
                alpha += 0.5 * atan(hw42_im[i+2][i]/hw42_re[i+2][i]);
